...
import ase
import numpy as np
from prody import AtomGroup, ClustENM
import torch
...
class PDBReader(ConfReader):
    ...
    """
    def __init__(self, pdb_file, dry=False, capped=False, stdaa=False, force_polarh=False, addHs=False, **kwargs):
    ...
    def prody_parser(self) -> AtomGroup:
        if self._prody_parser is None:
            ...
            if self.addHs:
                ...
                clustenm = ClustENM()
                clustenm.setAtoms(self._prody_parser)
...
class PDBHeteroReader(PDBReader):
    ...
    """
    def __init__(self, pdb_file, ion_pdb_file=None, **kwargs):
        self.ion_pdb_file = ion_pdb_file
    ...
    def protein_ag(self) -> AtomGroup:
        return self.prody_parser.protein.toAtomGroup()

    @lazy_property
    def water_ag(self) -> Optional[AtomGroup]:
        sel = self.prody_parser.copy().water
        if sel is None:
            return None
    ...
    def ion_ag(self) -> Optional[AtomGroup]:
        if self.ion_pdb_file is not None:
            return parsePDB(self.ion_pdb_file)
        
        sel = self.prody_parser.copy().ion
        if sel is None:
            return None
        return sel.toAtomGroup()
