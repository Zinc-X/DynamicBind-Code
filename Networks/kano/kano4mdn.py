...
class KanoAtomEmbed(nn.Module):
    def __init__(self, cfg: Config) -> None:
        ...
        if cfg.model.kano.kano_ckpt is not None:
            ...
            kano_ckpt = cfg.model.kano.kano_ckpt
            if osp.exists("/ai/share/workspace/sxia/ATSENSECORE.txt"):
                kano_ckpt = kano_ckpt.replace("/ai/share/workspace/songxia/", "/ai/share/workspace/sxia/scripts/")
            self.load_state_dict(torch.load(kano_ckpt, map_location=get_device()), strict=False)
        self.cfg = cfg
