...
def cal_PI(df):
    ...
    for i in range(0, 5):
        xi = lst[i]
        score = float(dfsorted.iloc[i]['score'])
        bindaff = float(dfsorted.iloc[i]['logKa'])
...
for i, j in group.__iter__():
    ...
    testdf2['logKa'] = pd.to_numeric(testdf2['logKa'], errors='coerce')
    testdf2['score'] = pd.to_numeric(testdf2['score'], errors='coerce')
    testdf2.dropna(subset=['logKa', 'score'], inplace=True)
    tartemp = top(testdf2)['#code'].tolist()
    tar = ''.join(tartemp)
    if len(testdf2) == 5:
        spearman.loc[tar, 'spearman'] = testdf2[['logKa', 'score']].corr(method='spearman').loc['logKa', 'score']
        kendall.loc[tar, 'kendall'] = testdf2[['logKa', 'score']].corr(method='kendall').loc['logKa', 'score']
