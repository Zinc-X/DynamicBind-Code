...
class MDNLossFn(BaseLossFn):
    ...
    def __call__(self, model_output, data_batch, is_training, loss_detail=False, mol_lvl_detail=False):
        ...
        detail.update(self.get_external_mdn(pair_prob, model_output, data_batch))
    ...
    def get_external_mdn(self, pair_prob, model_output, data_batch) -> dict:
        from Networks.PairedPropLayers.ProtSingleMDNLayer import IDX2BOND
        if not self.compute_external_mdn:
            return {}
        
        if "edge_type" not in model_output.keys():
            return self.nmdn_calculator(pair_prob, model_output, data_batch)
        
        # NMDN score by edge_type
        all_nmdn_types = set(model_output["edge_type"].detach().cpu().numpy().tolist())
        res = {}
        for nmdn_type in all_nmdn_types:
            mask = (model_output["edge_type"] == nmdn_type)
            type_name = IDX2BOND[nmdn_type]
            this_pair_prob = pair_prob[mask]
            this_model_output = {}
            for key in ["pi", "sigma", "mu", "dist", "C_batch"]:
                this_model_output[key] = model_output[key][mask]
            this_nmdn: dict = self.nmdn_calculator(this_pair_prob, this_model_output, data_batch)
            for key in this_nmdn:
                res[f"{type_name}_{key}"] = this_nmdn[key]
        return res
